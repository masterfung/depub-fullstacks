...
export default class Moderation {
    ...
    verify = async (text: string, artifacts: string[]) => {

        try {
            const prompt = this.rules.concat(`text: ${text}, labels: ${artifacts.join(";\n")}`).join("\n");
            ...
            const retval = Number.parseFloat(response.data.choices[0].text?.split("\n").at(-1) ?? "NaN");

            if (!isNaN(retval))
                return {success: true, confidence: retval};
